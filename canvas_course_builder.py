...
class CourseBuilder:
    ...
    def build(self, modules: list[Module], dry_run: bool = False):
        ...
        has_existing = any(
            m.canvas_module_id or any(
                getattr(item, 'canvas_module_item_id', None) 
                for item in m.items
            )
            for m in modules
        )
        ...
        for i, module in enumerate(modules, start=1):
            ...
            if module.canvas_module_id:
                ...
                print(f"  ✓ Updated module (ID: {module.canvas_id})")
            else:
    ...
    def _create_or_update_item(self, module: Module, item):
        ...
        if isinstance(item, TextHeader):
            ...
            if item.canvas_page_id:
                ...
                print(f"  ✓ [page] {item.title} (updated)")
            else:
                # Create new page
    ...
    def _preview(self, modules: list[Module]):
        ...
        for i, module in enumerate(modules, start=1):
            mode = "UPDATE" if module.canvas_module_id else "CREATE"
            print(f"\n[Module {i}] {module.title} ({mode})")
            
            for item in module.items:
                item_mode = "update" if getattr(item, 'canvas_module_item_id', None) else "create"
                ...
                if isinstance(item, TextHeader):
                    ...
                    page_mode = "update" if item.canvas_page_id else "create"
                    ...
                    file_mode = "update" if getattr(item, 'canvas_file_id', None) else "lookup"
                    print(f"  • [file] {item.title} ({file_mode})")
                    print(f"      Filename: {item.filename}")
                
                elif isinstance(item, Assignment):
                    assgn_mode = "update" if getattr(item, 'canvas_assignment_id', None) else "create"
                    ...
                    disc_mode = "update" if getattr(item, 'canvas_discussion_id', None) else "create"
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("=" * 60)
        print("\nUsage: python canvas_course_builder.py <markdown_file> [--dry-run]")
    ...
    if dry_run:
        # Preview only
        builder = CourseBuilder(None)
        builder._preview(modules)
        print("\n" + "=" * 60)
        print("This was a dry run. No changes were made.")
        print("Remove --dry-run to create content in Canvas.")
        print("=" * 60)
    else:
        # Get API token
        ...
        # Confirm
        print(f"\nThis will create {len(modules)} modules with {sum(len(m.items) for m in modules)} items.")
